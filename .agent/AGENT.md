...
- Mỗi asset phải có Rights Ledger và trạng thái APPROVED trước khi vào timeline cuối.
...
- Nếu title quá dài, rút gọn tên thư mục nhưng phải giữ đúng ý chính; title public đầy đủ vẫn được lưu trong Project Brief.
- Nếu đã có thư mục cùng tên, không ghi đè. Phải xác nhận đó có phải cùng project; nếu không, yêu cầu một tên phân biệt.
- Khi title thay đổi, cập nhật Project Brief trước. Chỉ đổi tên thư mục khi việc đổi tên không làm mất hoặc ghi đè dữ liệu.
...
Chỉ tạo những thư mục/file cần cho giai đoạn hiện tại, nhưng không đặt sai loại artifact sang khu vực khác.

### 7.3. Project Brief bắt buộc

**00_Project_Brief.md** phải ghi:

- public title hiện tại;
- working title trước đó, nếu có;
- project status;
...
- target duration;
- owner/reviewer;
- created date và last updated;
- link đến Source Table, Claim Ledger, Rights Ledger, script và checklist;
- quyết định GO/HOLD/REJECT tại từng gate.

## 8. Đầu ra research bắt buộc trong từng project

Trước khi viết annotated script, project phải có:

1. **Source Table** — nguồn, provenance, independence, methodology, freshness và claim fit.
2. **Claim Ledger** — claim, epistemic type, narrative role, source, confidence, counterpoint và trạng thái.
3. **Rights Ledger** — asset, owner, license, commercial/derivative rights, attribution, proof và trạng thái.

Trước khi tạo ElevenLabs:

- **Annotated_Script.md** phải qua **docs/Check_List.md**;
- không còn blocker;
- **Clean_VO.md** không chứa citation, visual tag hoặc editor note;
- mọi factual wording trong Clean VO phải truy ngược được về Claim Ledger.

Trước khi publish:

- Rights Ledger không còn asset chưa APPROVED trên timeline;
- Final Checklist là GO;
...
Trong annotated script, dùng tối thiểu:
...
Không đưa citation vào Clean VO trừ khi narration chủ động gọi tên nguồn vì lý do biên tập.
...
- [ ] Source Table, Claim Ledger và Rights Ledger được cập nhật khi có thay đổi.
